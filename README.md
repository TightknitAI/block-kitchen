...
| `updateButtonLabel` | `string` | no | Label for the toolbar's primary button while a message is loaded for editing (it opens the update dialog). Defaults to `'Review & update'`. When edit mode is on with no message loaded, the button reads `'Review & send'` and carries a split menu ("Edit a message" / "Send as a new message"). |
